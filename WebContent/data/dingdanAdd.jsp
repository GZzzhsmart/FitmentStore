...
<meta name="keywords" content="订单提交：家具,家具网,买家具,网购家具,网上家具商城,网上买家具">
<meta name="description" content="订单提交：网上家具商城，在线销售田园家具，韩式家具，美式家具，儿童家具，现代家具，中式家具，欧式家具，板式家具，实木家具，沙发，软床，衣柜，餐桌。">
...
<form action="<%=basePath %>dingdanservlet?order=add2&homeid=${home.homeid }" method="post" name="theForm" id="theForm"> 
<div class="w mt10"><h1 class="fl_h1">确认订单信息</h1></div>
<div class="pre_box w">
	<a id="address_md" name="address_md"></a>
	<div id="JS_address_box__" class="currentBox">
		<div class="title clearfix" name="address_box" id="address_box" style="background:#f0f0f0">
			<div class="text Left">收货人信息</div>
			<div class="more Right"><a href="<%=basePath %>addressservlet?order=init"  target="_blank" class="back_to_crat orange"><span class="icon"></span>新增收货地址</a></div>
		</div>
		<div class="body" id="JS_edit_address_edit">
			<div class="address_box" id="JS_address_list_box">
				<div style="padding-bottom:10px;" id="JS_after_new_address">使用地址簿中的地址：</div>
					<div id="JS_address_list_1580151" class="list" onmouseover="M.addClass(this,&#39;hover&#39;);" onmouseout="M.removeClass(this,&#39;hover&#39;);">
					<label>
						<span id="JS_address_list_detail_1580151"> <b>${address.username }</b>  ${address.address }  ${address.telephone }  </span>
					</label>
		...
		<div class="body" id="JS_edit_address_show" style="display:none;padding:0;padding-left:40px;position:absolute;margin-top:-22px;"></div>
	</div>
			...
			<table id="JS_cart_table" class="cart_table">
				...
				<tbody id="JS_shop_list_7844" class="shop_list" data-shop_id="7844">
					...
					<td class="first" style="line-height:1.5;width:110px;text-align:center;padding-left:5px;">
						<a href="<%=basePath %>homeservlet?order=look&homeid=${home.homeid }" target="_blank"><img class="img" src="<%=basePath%>${home.pictureurl }" width="90" height="58"></a>
					</td>
					<td style="text-align:left;vertical-align:top" valign="top">
						 
						<a style="line-height:18px" href="<%=basePath %>homeservlet?order=look&homeid=${home.homeid }" target="_blank">${home.homename }</a>
												  						 
						 
					<p class="gray">编号：L JJ-${home.homeid }</p>																		
					

					</td>
					<td class="yen">¥${home.price }<div class="goods_install_fee goods_install_single" style="display:none;"></div></td>
					<td class="yen"> <span class="goods_number">${shuliang }</span> 个 </td>
					<td class="yen">¥${home.discount }</td>
					<td><div class="yen" id="goods_subtotal_27542860">¥<span class="show_price">${(home.price -home.discount)*shuliang}</span></div><div class="goods_install_fee goods_install_end" style="display:none;"></div></td>
				...
				<tr class="count-area">
					<td colspan="8">
						<ul class="count-list">
							<li class="clearfix">
								<div class="col-left">商品小计：</div>
								<div class="col-right"><span id="JS_total_7844" class="total red"><span class="yen">¥</span>${(home.price -home.discount)*shuliang}</span></div>
							</li>
							<li class="clearfix">
								<div class="col-left">
								 <span data-shopid="7844">运费</span>：
								</div>
								<div class="col-right">
							...
							<li class="clearfix">
								<div class="col-left" data="">
									<span class="goods_activity_box none"><b>活动：</b>
									<select id="JS_goods_activity_7844" name="1" class="goods_activity" style="width: 140px;"><option value="0">选择活动</option></select></span>
									<span> 优惠：</span>
									<ul class="goods_activity_gift" style="display: none;"></ul>
								</div>
								<div class="col-right"><span class="cut" zcut="0"><span class="yen">¥${home.discount}</div>
							</li>
														<li class="clearfix">
								<div class="col-left">店铺小计：</div>
								<div class="col-right"><b id="JS_after_7844" class="red yen f16" data-total="2550"><span class="yen">¥</span>${(home.price -home.discount)*shuliang}</b></div>
							</li>
						</ul>

						<ul class="extend" style="clear:both">
							<li class="bonus">
								<div class="label"><span id="JS_user_bonus_handler_7844">使用红包抵消部分金额</span></div>
								<div id="JS_bonus_box_7844" class="theme">
									<div id="JS_bonus_table_box_7844" class="list"><table id="JS_bonus_table__7844"><tbody><tr><th>红包名称</th><th>红包金额</th><th>指定商品总金额</th><th>最小订单总金额</th><th>截止使用日期</th><th>同类型红包可否一起使用</th><th></th></tr><tr><td colspan="6" class="c"><span class="gray">您没有可用红包！</span></td></tr></tbody></table></div>
									<div class="coupon"><span>使用优惠券或线下红包，请输入序列号：</span><input class="youhui_sn" id="JS_youhui_sn_7844" style="vertical-align:top;"><a href="javascript:;" class="btn add" onclick="bonusBox._7844.useBonusById(&#39;JS_youhui_sn_7844&#39;);return false">添加</a><span style="color:#FF6600;padding-left:15px;">亲，使用红包后，红包抵扣金额将不再参与活动计算哦!</span></div>
								</div>
							</li>
							<li class="message open" id="li1">
								<div class="label"><span class="JS_cut">乐币抵扣</span></div>
								
							</li>
							<li class="message open" id="li2" style="display:none">
								<div class="theme"><span class="JS_cut" id="span1">已有乐币:${user.ledou }个</span></div>
								<div class="theme"><span class="JS_cut">请输入使用多少乐币:<input type="text" id="lebi" style="width:80px;border-width:1px;"/></span></div>						
								<div class="theme"><a href="javascript:void(0);"><input id="btn_shiyong" value="使用乐币" readonly="readonly"  style="width:100px;height:30px;text-align: center; font-size:22px" size="65"/></a></div>
							</li>
							<li class="message open">
								<div class="label"><span class="JS_cut">给卖家留言</span></div>
								<div class="theme">
									<div><textarea class="cus_comment" name="liuyan"></textarea></div>
									
								</div>
							</li>
					
							<li class="reminder" style="color: #999;padding-left: 12px;font-family: 微软雅黑;line-height: 12px;">
								<p>温馨提示：请您确认好下单商品的尺寸是否可以上楼、进门、摆放。</p>
							</li>
	...
	<div class="cart_extra" style="border:none;">
		<div class="clearfix">
		<table>
			<tbody>
				<tr>
					<td class="key">店铺总计：</td>
					<td class="val"><span class="yen" id="JS_shop_total"><span class="yen">¥</span>${(home.price -home.discount)*shuliang}</span></td>
				</tr>
				<tr class="spe">
					<td class="key"><strong class="f14">应付总额</strong>：</td>
					<td class="val"><strong class="red yen f18" id="JS_after">¥${(home.price -home.discount)*shuliang}</strong></td>
				</tr>
			</tbody>
		</table>
		</div>
...
<span id="JS_submit_msg" style="color:#555"></span>
<button id="tijiao" style="height: 44px; width: 180px; background-color: #f7041d ;font-size: 20px"><font color="white">提 交 订 单</font></button>
</div>
 <input type="hidden" name="money" id="money"/>
 <input type="hidden" name="shuliang" id="shuliang" value="${shuliang }"/>
 <input type="hidden" name="money1" id="money1" value="${(home.price -home.discount)*shuliang }"/>
 <input type="hidden" name="money2" id="money2" value="${user.ledou }"/>
</form>


<textarea class="none" id="JS-add-buy-show"></textarea>
...
<div class="footer">
	...
	<script type="text/javascript">
		$(function(){
			...
			$("#btn_shiyong").bind("click",function(){
				...
				if(confirm('您确定要使用乐币？')){
					...
					if(libi>${user.ledou}){
						...
						return false;
							...
							var summoney = document.getElementById("JS_after").innerHTML;
							document.getElementById("JS_after").innerHTML="¥"+(${(home.price -home.discount)*shuliang}-libi)+".0";
							document.getElementById("span1").innerHTML="已有乐币:"+(${user.ledou}-libi)+"个";
							alert("已使用"+libi+"个乐币，请确认");
							document.getElementById("money").value="0";
							$("#money1").val(${(home.price -home.discount)*shuliang}-libi);
							$("#money2").val(${user.ledou}-libi);
							return true;
